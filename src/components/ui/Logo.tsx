 ...
 */
export function PrismSVG({ size = 32, className, noGlow = false, idPrefix }: PrismProps) {
  ...
  return (
    ...
    >
      ...
      <g filter={noGlow ? undefined : `url(#${id}-glow)`}>
        <polygon points={TOP_FACET}   fill={`url(#${id}-top)`}   stroke="white" strokeWidth="1.6" strokeLinejoin="round" />
        <polygon points={RIGHT_FACET} fill={`url(#${id}-right)`} stroke="white" strokeWidth="1.6" strokeLinejoin="round" />
        <polygon points={LEFT_FACET}  fill={`url(#${id}-left)`}  stroke="white" strokeWidth="1.6" strokeLinejoin="round" />
      </g>
...
export function LogoWordmark({ height = 24, className, color = '#FFFFFF' }: WordmarkProps) {
  return (
    <span
      className={className}
      style={{
