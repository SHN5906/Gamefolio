...
import type { GameCard, Pack } from '@/types/game'
import { rollCard } from '@/data/packs'
import { useTCGdexCard, tcgdexImageUrl, useTCGdexCards } from '@/hooks/useTCGdexCard'
...
export function CaseOpener({ pack, onComplete, triggerKey }: CaseOpenerProps) {
  ...
  const animConfigRef = useRef<{ targetX: number; jitter: number }>({ targetX: 0, jitter: 0 })
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < RAIL_LENGTH; i++) {
      if (i === WINNER_INDEX) {
        ...
        const randomCard = pack.cardPool[Math.floor(Math.random() * pack.cardPool.length)]
    ...
    const containerWidth = typeof window !== 'undefined' ? Math.min(window.innerWidth, 1100) : 1100
    ...
    <div className="relative w-full">
      ...
      >
        ...
        <div className="absolute left-1/2 top-0 bottom-0 -translate-x-1/2 z-20 pointer-events-none">
          ...
          >
            <ChevronDown size={28} strokeWidth={3} style={{ color: 'var(--color-pokemon-yellow)' }} />
          </div>
          <div
        ...
        <div
          ...
          style={{ background: 'linear-gradient(90deg, var(--color-bg-elevated), transparent)' }}
        />
        <div
 ...
 */
function CardFallback({ card, displayName, size }: {
  card: GameCard
  displayName: string
  size: 'small' | 'large'
}) {
  const isLegendary = ['shining', 'gold-star', 'crystal', 'secret-rare', 'rainbow-rare'].includes(card.rarity)
...
function WinnerReveal({ winner }: { winner: GameCard }) {
      ...
      >
        ...
        >
          {imageUrl ? (
            ...
            <CardFallback card={winner} displayName={displayName} size="large" />
          )}
          <div
