...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (env.isDemoMode) {
      ...
      const demo: ProfileRow = {
        ...
        username: "hugo_marceau",
        display_name: "Hugo Marceau",
        avatar_url: null,
        default_currency: "EUR",
        default_language: "fr",
        subscription_tier: "pro",
        subscription_renews_at: "2026-12-31",
  ...
  async function handleSaveProfile() {
    ...
    try {
      if (env.isDemoMode) {
        ...
        setSavedMsg("Profil mis à jour (démo)");
        return;
      }
  ...
  async function handleSaveEmail() {
    ...
    try {
      if (env.isDemoMode) {
        await new Promise((r) => setTimeout(r, 400));
        setSavedMsg("Email mis à jour (démo)");
        return;
      }
  ...
  async function handleSavePassword() {
    ...
    try {
      if (env.isDemoMode) {
        await new Promise((r) => setTimeout(r, 400));
        setSavedMsg("Mot de passe changé (démo)");
        setNewPassword("");
        return;
      }
  ...
  return (
    <div
      className="px-4 sm:px-6 md:px-8 pt-6 md:pt-7 pb-12 mx-auto"
      style={{ maxWidth: 800 }}
    >
      {/* Header */}
      <div className="mb-7">
      ...
      {savedMsg && (
        <div
          className="flex items-center gap-2 px-4 py-2.5 rounded-[var(--radius-sm)] mb-4 text-[12px]"
          style={{
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, var(--color-pokemon-yellow), var(--color-warning))",
              }}
            >
      ...
      <Section
        icon={User}
        title="Profil public"
        subtitle="Ton pseudo et avatar — visibles dans les battles, jackpot et leaderboard"
      >
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <div className="flex-1 flex flex-col gap-3">
            <Input
              ...
              placeholder="Dresseur Pro"
            />
            <Input
              label="Username (URL publique)"
              value={username}
              onChange={(e) =>
                setUsername(
      ...
      <Section
        icon={Mail}
        title="Email"
        subtitle="Utilisé pour les notifications et la connexion"
      >
              ...
              style={{
                background:
                  currency === c
                    ...
                    : "var(--color-bg-glass)",
                borderColor:
                  currency === c ? "var(--color-brand)" : "var(--color-border)",
                color: currency === c ? "white" : "var(--color-text-secondary)",
                boxShadow:
                  currency === c ? "0 0 12px var(--color-brand-glow)" : "none",
              }}
            >
      ...
      <Section
        icon={Bell}
        title="Notifications"
        subtitle="Reste informé sans spam"
      >
        <ToggleRow
          label="Email quand un battle PvP que j'attends démarre"
          defaultOn
        />
      ...
      <Section icon={Eye} title="Confidentialité">
        <ToggleRow
          label="Profil public visible (battles, jackpot, leaderboard)"
          defaultOn
        />
    ...
    <div className="flex items-center justify-between py-2">
      ...
      <button
        ...
        style={{
          ...
          boxShadow: on
            ? "inset 0 0 0 1px var(--color-brand-hi), 0 0 12px var(--color-brand-glow)"
