...
const BUNDLES: ShopBundle[] = [
  { id: 'small',  amount: 5,   price: 4.99,   bonus: 0,  icon: <Zap size={18} />,   color: '#5B7FFF', glow: 'rgba(91,127,255,0.35)' },
  { id: 'medium', amount: 12,  price: 9.99,   bonus: 20, icon: <Star size={18} />,  color: '#10D9A0', glow: 'rgba(16,217,160,0.35)', popular: true },
  { id: 'large',  amount: 30,  price: 19.99,  bonus: 50, icon: <Gem size={18} />,   color: '#A855F7', glow: 'rgba(168,85,247,0.35)' },
  { id: 'huge',   amount: 80,  price: 49.99,  bonus: 60, icon: <Crown size={18} />, color: '#FFD700', glow: 'rgba(255,215,0,0.4)', best: true },
  { id: 'mega',   amount: 200, price: 99.99,  bonus: 100, icon: <Crown size={18} />, color: '#EC4899', glow: 'rgba(236,72,153,0.4)' },
]
    ...
    <div className="min-h-full page-enter" style={{ position: 'relative', zIndex: 1 }}>
      <div className="px-4 sm:px-6 md:px-8 pt-6 pb-16 mx-auto" style={{ maxWidth: 1100 }}>
        ...
        <div className="flex items-end justify-between mb-7">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <ShoppingCart size={14} style={{ color: 'var(--color-purple)' }} />
              <p
            ...
            <p className="text-[13px] mt-2" style={{ color: 'var(--color-text-secondary)' }}>
              Solde actuel : <strong style={{ color: 'var(--color-positive)' }}>${balance.toFixed(2)}</strong>
              {' · '}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {BUNDLES.map(b => (
            <BundleCard key={b.id} bundle={b} onPurchase={() => handlePurchase(b)} />
          ))}
        </div>
        ...
        >
          ...
          <ul className="space-y-2 text-[12px]" style={{ color: 'var(--color-text-secondary)' }}>
            <li>• Toutes les cartes sont accessibles via les caisses gratuites + missions</li>
            <li>• Aucune carte exclusive aux joueurs payants</li>
            <li>• Les achats donnent juste plus d&apos;ouvertures, pas d&apos;avantage</li>
...
function BundleCard({
  ...
  return (
    <div
      ...
      style={{
        background: bundle.best || bundle.popular
    ...
    >
      ...
      {bundle.bonus > 0 && (
        ...
        >
          ${bundle.amount.toFixed(2)} + ${(bundle.amount * bundle.bonus / 100).toFixed(2)} bonus ({bundle.bonus}%)
        </p>
      )}
