...
import { ArrowLeft, Trophy, Users, Plus, History } from 'lucide-react'
...
const MOCK_DEPOSITS: MockDeposit[] = [
  { initial: 'L', username: 'LiamPro',     value: 28.50, cardCount: 3, color: '#5B7FFF' },
  { initial: 'M', username: 'MewMaster',   value: 18.20, cardCount: 5, color: '#10D9A0' },
  { initial: 'S', username: 'ShinyKing',   value: 14.80, cardCount: 2, color: '#FFCC00' },
  { initial: 'R', username: 'RainbowGod',  value: 9.50,  cardCount: 4, color: '#EC4899' },
  { initial: 'D', username: 'DraGoat',     value: 6.00,  cardCount: 2, color: '#FF4D5E' },
]
...
function getEndsIn(): string {
  ...
  target.setSeconds(0)
  if (target.getTime() <= now.getTime()) target.setMinutes(target.getMinutes() + 5)
        ...
        <div className="flex items-end justify-between mb-7">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Trophy size={14} style={{ color: 'var(--color-pokemon-yellow)' }} />
              <p
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        {d.cardCount} carte{d.cardCount > 1 ? 's' : ''} · ${d.value.toFixed(2)}
                      </p>
                    </div>
                    {/* Mini bar */}
...
function Donut({ deposits, totalPot }: { deposits: MockDeposit[]; totalPot: number }) {
  const size = 220
