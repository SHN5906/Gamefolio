...
import { Sparkles, Swords, Trophy, Gift, ShoppingCart, ArrowUpRight } from 'lucide-react'
...
export default function GamePage() {
  ...
  const tier1 = PACKS.filter(p => p.tier === 'starter' || p.tier === 'common')
  ...
  return (
    <div className="min-h-full page-enter" style={{ position: 'relative', zIndex: 1 }}>
      <div className="px-4 sm:px-6 md:px-8 pt-6 pb-16 mx-auto" style={{ maxWidth: 1280 }}>
        ...
        {/* Welcome modal $10 */}
        {showWelcome && (
          <WelcomeBonus onClose={() => setShowWelcome(false)} />
        )}

        {/* Header */}
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-2">
            <Sparkles size={14} style={{ color: 'var(--color-pokemon-yellow)' }} />
            <p
          ...
          <p className="text-[13px] mt-2" style={{ color: 'var(--color-text-secondary)' }}>
            15 caisses, des centaines de cartes Pokémon, des jackpots à six chiffres. $10 offerts à l&apos;inscription.
          </p>
        </div>
        ...
        {/* Tier 1 */}
        <Section title="Caisses accessibles" subtitle="Idéales pour débuter — moins de $2">
          <Grid>
            {tier1.map(pack => (
              <PackCard
                key={pack.id}
        ...
        {/* Tier 2 */}
        <Section title="Standard" subtitle="Légendaires, GX, Tag Team — $2 à $5">
          <Grid>
            {tier2.map(pack => (
              <PackCard
                key={pack.id}
        ...
        {/* Tier 3 */}
        <Section title="Premium" subtitle="Shinings, Cristals, Gold Star — collectors de niche">
          <Grid>
            {tier3.map(pack => (
              <PackCard
                key={pack.id}
    ...
    >
      ...
      >
        ...
        <p className="text-[14px] mb-5" style={{ color: 'var(--color-text-secondary)' }}>
          On t&apos;offre <strong style={{ color: 'var(--color-positive)' }}>$10</strong> pour démarrer.
          De quoi ouvrir <strong style={{ color: 'var(--color-pokemon-yellow)' }}>jusqu&apos;à 30 caisses</strong>. Lance-toi.
        </p>
        <button
          onClick={onClose}
