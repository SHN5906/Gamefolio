...
export default async function PublicProfilePage({ params }: PageProps) {
  ...
  const totalValue = cards.reduce((acc, c) => acc + (c.current_price_eur ?? 0) * c.quantity, 0)
...
function DemoProfile({ username }: { username: string }) {
  ...
  const totalValue = MOCK_CARDS.reduce((acc: number, c: { value: number }) => acc + c.value, 0)
...
function PublicProfileLayout({ displayName, username, avatarUrl, memberSince, tier, totalValue, cardCount, setCount, cards }: LayoutProps) {
  const initial = displayName[0]?.toUpperCase() ?? '?'

  return (
    <div className="min-h-screen relative" style={{ zIndex: 1 }}>
      {/* Top banner */}
      <div className="px-4 sm:px-6 md:px-8 py-3 border-b sticky top-0 z-20"
        style={{
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-end gap-5 mb-8">
            ...
            <div
              ...
              style={{
                fontFamily: 'var(--font-display)',
                background: avatarUrl ? '#0B0F1A' : 'linear-gradient(140deg, var(--color-brand), var(--color-cyan))',
              ...
              <div className="flex items-center gap-2 mb-1.5">
                ...
                {tier !== 'free' && (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider"
                    style={{ background: 'var(--color-pokemon-yellow-soft)', color: 'var(--color-pokemon-yellow)', border: '1px solid rgba(255,204,0,0.3)' }}>
                    <Sparkles size={9} /> {tier}
                  </span>
                )}
              </div>
              <h1 className="text-[36px] font-extrabold leading-none tracking-tight"
                style={{ fontFamily: 'var(--font-display)', letterSpacing: '-0.025em' }}>
                {displayName}
              </h1>
              <p className="text-[12px] mt-2 font-mono" style={{ color: 'var(--color-text-muted)' }}>
                @{username} · Membre depuis {new Date(memberSince).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-px rounded-[var(--radius)] overflow-hidden border"
            ...
            {[
              { lbl: 'Valeur estimée',  val: formatEur(totalValue),   sub: 'cotation Cardmarket' },
              { lbl: 'Cartes',           val: String(cardCount),       sub: `${setCount} sets` },
              { lbl: 'Cartes gradées',   val: String(cards.filter(c => c.grade).length), sub: 'PSA' },
              { lbl: 'Top asset',        val: cards.sort((a,b)=>b.value-a.value)[0]?.name?.split(' ').slice(0,2).join(' ') ?? '—', sub: cards[0] ? formatEur(cards[0].value) : '' },
            ].map(({ lbl, val, sub }) => (
              <div key={lbl} className="px-4 py-3" style={{ background: 'var(--color-bg-glass)' }}>
                <p className="text-[9.5px] font-semibold uppercase tracking-[0.9px] mb-1" style={{ color: 'var(--color-text-muted)' }}>{lbl}</p>
                <p className="text-[16px] font-bold leading-none truncate" style={{ fontFamily: 'var(--font-display)', letterSpacing: '-0.02em' }}>
                  {val}
                </p>
                <p className="text-[10px] mt-1" style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-text-muted)' }}>{sub}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-12">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[18px] font-bold tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>
              {cardCount} cartes
            </h2>
          ...
          <div className="grid gap-2 sm:gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(135px, 1fr))' }}>
            {cards.map(c => <CollectionCard key={c.id} card={c} />)}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-16">
        <div className="max-w-3xl mx-auto rounded-[var(--radius-lg)] border p-7 text-center relative overflow-hidden"
          style={{
            background: 'linear-gradient(135deg, rgba(42,125,255,0.1) 0%, rgba(0,212,255,0.05) 50%, rgba(11,15,26,0.6) 100%)',
            borderColor: 'var(--color-border-strong)',
          }}>
          <div className="absolute inset-x-0 top-0 h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(42,125,255,0.5), transparent)' }} />
          <h3 className="text-[24px] font-extrabold tracking-tight mb-2" style={{ fontFamily: 'var(--font-display)', letterSpacing: '-0.025em' }}>
            Crée ton propre portfolio Pokémon
          </h3>
          <p className="text-[13px] mb-5" style={{ color: 'var(--color-text-secondary)' }}>
            Suis la valeur de ta collection en temps réel. Gratuit pour démarrer.
          </p>
          <Link
            href="/signup"
            ...
            style={{ background: 'linear-gradient(135deg, var(--color-brand), var(--color-cyan))', boxShadow: '0 0 24px var(--color-brand-glow)' }}
          >
